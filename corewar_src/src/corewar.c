...
/*   Updated: 2017/03/15 16:14:10 by bandre           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
TTF_Font	*start_sdl(SDL_Surface **ecran)
{
	...
	TTF_Init();
	if (!(police = TTF_OpenFont("corewar_src/test.ttf", 15)))
		quit_clean(4);
	if (!(*ecran = SDL_SetVideoMode(1750, 1150, 32, SDL_HWSURFACE)))
		quit_clean(4);
