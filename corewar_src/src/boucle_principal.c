...
/*   Updated: 2017/03/14 22:35:01 by bandre           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void		op_pross(int (*ft_op[16])(t_process *proc),
		SDL_Surface *ecran, TTF_Font *police, t_option option)
{
	t_process	*process;
	int			retour;
	...
	while (process)
	{
		if (process->cycle == 0 && process->op_code > 0
				&& process->op_code < 17)
		{
			process->act_cycle = option.total_cycles;
			retour = ft_op[process->op_code - 1](process);
			if (retour != -1 && option.visu == 1)
		...
		TTF_Font *police, t_option option)
{
	...
	ft_define_op(ft_op);
	list_process();
	option.total_cycles = 0;
	op_pross(ft_op, ecran, police, option);
	while (cycles_to_die > 0 && g_list_process)
	{
		if (option.total_cycles == option.dump)
			print_memory();
		...
		option.total_cycles++;
		list_process();
		if (gestion_event(option, option.total_cycles, ecran, police) == 0)
			return (1);
	}
	return (1);
}
